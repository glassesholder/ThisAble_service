...
def CPT():
    ...
    system_template_hint = """당신은 특수교육 대상자를 위한 상담선생님 입니다.
    사용자는 특수교육 대상자이며, 정신 연령은 9~12세 입니다.
    당신은 사용자의 질문에 친절하게 상담해줘야 하며, 쉬운 단어를 사용해 답변해주세요.
    답변에 부정적인 단어를 포함해서는 안됩니다.
    사용자가 친절함을 느낄 수 있도록 최대한 친절하게 사용자의 이야기에 공감하고 답변해주세요.
    답변은 2문장 이상으로 답변해 주세요.
    답변을 할 때마다 문장에 알맞는 감정 이모티콘을 1개 이상 꼭 함께 넣어주세요.
    사용자의 질문에 존댓말로 답변해야 합니다.
    ...
    st.caption('챗봇은 GPT-3.5-turbo 모델에 대화 데이터셋을 파인튜닝하여 제작되었습니다.')
    st.divider()
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = [{"role": "assistant", "content": "안녕하세요, 믿음님! 제가 들어드릴게요!"}]
    ...
    # 사용자 질문과 그에 따른 응답 출력
    if prompt := st.chat_input("믿음님의 고민을 적어주세요!"):  # 만약 사용자가 입력한 내용이 있다면 
        ...
        selected = pills("답변은 어떤가요?", ["만족해요", "답변이 불친절해요", "이해가 안 돼요"], ["👍", "👎", "❓"], index=False)